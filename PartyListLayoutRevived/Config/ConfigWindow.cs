...
using PartyListLayoutRevived.Converter;
using PartyListLayoutRevived.GameStructs.NumberArray;
using PartyListLayoutRevived.GameStructs.StringArray;
using PartyListLayoutRevived.Helper;
using Util = PartyListLayoutRevived.Helper.Util;
    ...
    public unsafe class ConfigWindow {
        ...
        private void ElementConfigEditor(string name, ElementConfig eCfg, ElementConfig defaultCfg, ref bool c, LayoutElementFlags flags) {
            ...
            if (isOpen) {
                ...
                eCfg.Editor(name, ref c, plugin.PartyListLayoutRevived);
        ...
        private void DrawWindow() {
            ...
            if (ImGui.Begin($"{plugin.Name} Config", ref isOpen)) {
                ...
                if (!string.IsNullOrEmpty(importError)) {
                    ...
                    var exportBtnSize = ImGuiHelpers.GetButtonSize("Export");
                    var importBtnSize = ImGuiHelpers.GetButtonSize("Import");
                    ImGui.SetCursorPosX(ImGui.GetContentRegionAvail().X - exportBtnSize.X - importBtnSize.X);
                ...
                ImGui.BeginChild("PartyListLayoutRevived_scroll", new Vector2(-1, -1), false);

                var tabHeight = ImGui.CalcTextSize("A").Y + (2 * ImGui.GetStyle().FramePadding.Y);


                ImGui.PushStyleVar(ImGuiStyleVar.WindowPadding, Vector2.Zero);
                ImGui.BeginChild("PartyListLayoutRevived_tabSelect", new Vector2(-1, tabHeight), true, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse);
                ...
                ImGui.BeginChild("PartyListLayoutRevived_selectedTab", new Vector2(-1));
                ...
                switch (selectedTab) {
                    ...
                    case Tabs.Presets: {
                        ...
                        if (ImGui.BeginTable("PartyListLayoutRevived_presetTable", 4)) {
                ...
                try {
                    if (c) {
                        plugin.PartyListLayoutRevived.ConfigUpdated();
        ...
        private string GetConfigExport(bool uncompressed = false) {
            ...
            var compressedString = Util.Compress($"PartyListLayoutRevived::{json}");
        ...
        private LayoutConfig ImportConfig(string str, bool uncompressed = false) {
            try {
                ...
                if (uncompressed) {
                    ...
                    if (decompressedString.StartsWith("SimpleTweaks.UiAdjustments@PartyListLayoutRevived::")) {
                        decompressedString = decompressedString.Replace("SimpleTweaks.UiAdjustments@", "");
                    }

                    if (!decompressedString.StartsWith($"PartyListLayoutRevived::", StringComparison.InvariantCultureIgnoreCase)) {
                        SimpleLog.Log("Incorrect Identifier");
                        SimpleLog.Log($"{decompressedString}");
                        return null;
                    }

                    json = decompressedString.Substring($"PartyListLayoutRevived::".Length);
                }
